...
@Controller
public class NewsMediaOrderController extends BaseController {
	
	private static final Logger logger = LoggerFactory.getLogger( NewsMediaOrderController.class );
	
	@Autowired
	private NewsMediaOrderService newsMediaOrderService;
	...
	public Map<String, Object> list( BaseViewModel viewModel ) {
		...
		try {
			resData = newsMediaOrderService.getNewsMediaOrderList( viewmodel );
		} catch ( Exception e ) {
			logger.error( "查询新闻媒体订单列表失败!" + e );
	...
	public Map<String, Object> detail( BaseViewModel viewModel ) {
		...
		try {
			NewsMediaOrderDetailDTO orderDetail = newsMediaOrderService.getNewsMediaOrderDetail( orderId );
	...
	public Map<String, Object> orderSplitStatus( BaseViewModel viewModel ) {
		...
		try {
			BaseMediaOrderStatusDetailDTO orderStatusDetail = newsMediaOrderService.getOrderSplitStatusDetail( orderSplitId );
	...
	public Map<String, Object> editStatus( BaseViewModel viewModel ) {
		...
		try {
			Boolean ret = newsMediaOrderService.editNewsMediaOrderSplitStatus( orderSplitId, procType, rejectMsg );
			if ( !ret ) {
	...
	public Map<String, Object> editLaunch( BaseViewModel viewModel ) {
		...
		try {
			Boolean ret = newsMediaOrderService.editNewsMediaOrderLaunch( orderSplitId, launchUrl, launchDate );
			if ( !ret ) {
				return ContainerUtils.buildResFailMap( "操作失败!" );
			}
		} catch ( Exception e ) {
			logger.error( "查询新闻媒体投放详情失败!" + e );
			return ContainerUtils.buildResFailMap( "操作失败!" );
		}
	...
	public Map<String, Object> add( BaseViewModel viewModel ) {
		...
		try {
			...
			// valid inputs
			String retMsg = ValidOrderInputsUtils.validNewsMediaCommitOrderInputs( formData );
			if ( !retMsg.equals( "" ) ) {
				return ContainerUtils.buildResFailMap( retMsg );
			}
			
			NewsMediaCommitOrderViewmodel viewmodel = OrderDataTransforUtils.transNewsMediaCommitOrderByFormData( formData );
			...
			Integer resFlag = newsMediaOrderService.commitNewsMediaOrder( viewmodel );
			if ( resFlag == -2 ) {
				return ContainerUtils.buildResMap( resData, -2, "余额不足" );
			}
			
		} catch ( Exception e ) {
			logger.error( "新闻媒体下单失败!" + e );
			return ContainerUtils.buildResFailMap( "操作失败" );
		}
	...
	public Map<String, Object> pay( BaseViewModel viewModel ) {
		...
		try {
			Integer ret = newsMediaOrderService.payNewsMediaOrder( orderId );
			if ( ret == -2 ) {
				return ContainerUtils.buildResMap( resData, -2, "余额不足!" );
			} else if ( ret == -5 ) {
